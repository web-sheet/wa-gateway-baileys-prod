...
//     const apiKeyAdminPengirim =
//       "0bbf91e97a6beee2457fd7ba5fe20ab37ddf461604313ec022e37ebb2e808b5a";
...
//     console.log(
//       `[NOTIF ADMIN] Notifikasi status berhasil dikirim oleh Admin. Response:`,
//       response.data,
//     );
//     return true;
//   } catch (error) {
//     console.error(
//       "[NOTIF ADMIN] Gagal mengirim notifikasi via API Admin:",
...
export function getDisconnectReason(code) {
  switch (code) {
    ...
    case DisconnectReason.connectionClosed:
      return "Koneksi ditutup secara sepihak oleh WhatsApp.";

    case DisconnectReason.connectionReplaced:
    case 411:
      return "Sesi digantikan (Akun WhatsApp sedang dibuka di server atau device lain).";

    case DisconnectReason.restartRequired:
    case 515:
      return "WhatsApp meminta restart stream koneksi.";

    case DisconnectReason.timedOut:
      return "Koneksi mencapai batas waktu (Timed Out).";

    case DisconnectReason.badSession:
      return "File sesi rusak atau tidak valid. Silakan hapus sesi dan scan QR ulang.";

    // Tambahan baru untuk kasus spesifik
    case 403:
      return "Akses ditolak (Forbidden). Akun mungkin terindikasi spam atau dibatasi.";
    case 500:
    case 502:
    case 503:
    case 504:
      return "Masalah pada server WhatsApp (Gateway Timeout/Error).";

    default:
      return code
        ? `Terputus dengan kode error: ${code}`
        : "Terputus tanpa kode error yang jelas (kemungkinan koneksi terputus tiba-tiba).";
  }
}
